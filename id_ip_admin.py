...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="flex mb-10">
      <input type="text" id="newUser" placeholder="username 或 数字ID" onkeydown="if(event.key==='Enter')addUser()">
      <button class="btn btn-primary btn-sm" onclick="addUser()">+ 添加</button>
    </div>
    <div id="msgArea"></div>
    <table>
      <thead><tr><th style="width:30px;">#</th><th>用户</th><th>类型</th><th style="width:80px;">操作</th></tr></thead>
...
async function loadWhitelist() {
  ...
  if (!d.users || d.users.length === 0) {
    tbody.innerHTML = '<tr><td colspan="4" class="empty">暂无白名单用户（所有人可用）</td></tr>';
    return;
  }
  tbody.innerHTML = d.users.map((u, i) => {
    const type = /^\d+$/.test(u) ? '数字ID' : '用户名';
    return `<tr><td>${i+1}</td><td>${u}</td><td><span class="tag">${type}</span></td><td><button class="btn btn-sm btn-danger" onclick="removeUser('${u}')">移除</button></td></tr>`;
  }).join('');
}
...
async function addUser() {
  ...
  if (!val) return;
  const r = await fetch('/api/whitelist/add', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({user: val}) });
  const d = await r.json();
...
def api_whitelist():
    wl = load_whitelist()
    return jsonify({"ok": True, "users": sorted(wl)})
...
def api_whitelist_add():
    ...
    save_whitelist(wl)
    _reload_bot()
    return jsonify({"ok": True, "msg": f"已添加 {user}"})
...
def load_whitelist():
    if WHITELIST_FILE.exists():
        try:
            return set(json.loads(WHITELIST_FILE.read_text()).get("users", []))
        except:
            return set()
    return set()
