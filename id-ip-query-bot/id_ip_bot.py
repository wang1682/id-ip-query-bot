...
def query_ip(ip):
    """查询IP归属地（chinaz 街道级，带缓存）"""
    import time as _t
    now = _t.time()
    if ip in _ip_cache and (now - _ip_cache[ip]["time"]) < _IP_CACHE_TTL:
        cached = _ip_cache[ip]
        return cached["loc"], cached["isp"], cached["extra"]
    
    apikey = "apiuser_quantity_58d1a8fd50574d9039ab9c81fcf8086b_d25f4718cfb841dcb37a43202298dcfb"
    apikey = "apiuser_quantity_58d1a8fd50574d9039ab9c81fcf8086b_d25f4718cfb841dcb37a43202298dcfb"
    try:
        r = requests.get(
            f"https://openapi.chinaz.net/v1/1048/newip?ip={ip}&APIKey={apikey}&ChinazVer=1.0",
            timeout=10
        )
        if r.status_code == 200:
            ...
            if data.get("code") == 200 and data.get("data"):
                ...
                street = loc.get("street", "") or (loc.get("multi_street", [{}])[0].get("street", "") if loc.get("multi_street") else "")
                lat = loc.get("latitude", "")
                lng = loc.get("longitude", "")
                isp = loc.get("isp", "")
                # 去重（直辖市 region=city）
                parts = []
                for p in [country, region, city, district]:
                    if p and p not in parts:
                        parts.append(p)
                addr = " ".join(parts)
                extras = []
                if lat and lng:
                    extras.append(f"{lat}, {lng}")
                if street:
                    extras.append(street)
                extra = " | ".join(extras)
                _ip_cache[ip] = {"loc": addr, "isp": isp, "extra": extra, "time": now}
                return addr, isp, extra
...
def format_result_single(item_type, value, data, card_name=""):
    """格式化单条结果（扁平格式，无树形符号）"""
    if item_type == "id":
        lines = []
        if card_name:
            lines.append("姓名：" + card_name)
        lines.append("身份证号：" + value)
        lines.append("地区：" + data["area"])
        lines.append("出生：" + data["birth"] + "（" + data["gender"] + "）")
        lines.append("状态：" + data["status"])
        return "\n".join(lines)
        ...
        if card_name:
            lines.append("姓名：" + card_name)
        lines.append("手机号：" + value)
        lines.append("归属地：" + (area or "查询失败"))
        if carrier:
            lines.append("运营商：" + carrier)
        return "\n".join(lines)
        return line
    else:
        loc, isp, extra = data
...
def format_verify_result(vtype: str, value: str, name: str, api_result: dict) -> str:
    ...
    # 标注来源
    channel = "华道" if vtype in ("运营商", "银行卡") else "燕骏云"
    line = f"{vtype}二要素：{status_icon} {desc} \u2022 {channel}"
    if bankname:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if str(user.id) != ADMIN_ID:
        ...
        if not session_username:
            text_pin = update.message.text.strip()
            if 4 <= len(text_pin) <= 64:
                import hashlib
                pin_hash = hashlib.sha256(text_pin.encode()).hexdigest()
                conn2 = sqlite3.connect(str(LOG_DB))
                row2 = conn2.execute("SELECT username FROM bot_users WHERE pin_hash = ? AND is_active = 1", (pin_hash,)).fetchone()
                conn2.close()
                if row2:
                    create_session(str(user.id), row2[0])
                    session_username = row2[0]
                    await update.message.reply_text(f"✅ 登录成功，欢迎 {session_username}")
                    return
            await update.message.reply_text("⚠️ 会话已过期，请重新登录：\n/login 用户名 PIN")
            return
    else:
        session_username = (user.username or "").strip().lower() or str(user.id)
    ...
    for num in id_nums:
        ...
        if data:
            iname = ""
            im = re.search(r'([一-鿿]{2,4})\s*' + re.escape(num), text)
            if im and im.group(1) not in ('查询', '结果', '手机', '电话', '身份证', '银行卡', '归属地', '运营商', '地区', '出生', '性别', '状态', '有效', '无效'):
                iname = im.group(1)
            results.append(format_result_single("id", num, data, iname))
            if iname:
                ir = verify_idcard(num, iname)
                results.append(format_verify_result("身份证", num, iname, ir))
    ...
    if len(names) == 1:
        ...
        # 只有1条身份证号+姓名 → 身份证二要素（验证结果由上方身份证循环统一输出）
        elif len(id_nums) == 1 and not phones and not ips and not bank_nums:
            pass  # 验证结果由上方 ID 循环（line 908-910）统一输出
        # 只有1条银行卡号+姓名 → 银行卡二要素（验证结果由下方银行卡详情循环统一输出）
        elif len(bank_nums) == 1 and not id_nums and not phones and not ips:
            pass  # 详情和验证结果由下方银行卡循环处理

        # 查询银行卡（通过二要素API显示卡片信息）
    for bnum in bank_nums:
        name_m = re.search(r'([\u4e00-\u9fff]{2,4})\s*' + re.escape(bnum), text)
        bname = name_m.group(1) if name_m and name_m.group(1) not in ('\u67e5\u8be2', '\u7ed3\u679c', '\u624b\u673a', '\u7535\u8bdd', '\u8eab\u4efd\u8bc1', '\u94f6\u884c\u5361', '\u5f52\u5c5e\u5730', '\u8fd0\u8425\u5546', '\u5730\u533a') else ""
        vr = verify_bankcard(bnum, bname)
        if vr.get("code") == 200:
            dd = vr.get("data", {})
            lines = []
            if bname:
                lines.append(f"\u59d3\u540d\uff1a{bname}")
            lines.append(f"\u94f6\u884c\u5361\uff1a{bnum}")
            bn = dd.get("bankname", "")
            ct = dd.get("bankcardtype", "")
            if bn:
                lines.append(f"\u53d1\u884c\u884c\uff1a{bn}")
            if ct:
                lines.append(f"\u5361\u7c7b\u578b\uff1a{ct}")
            results.append("\n".join(lines))
            results.append(format_verify_result("\u94f6\u884c\u5361", bnum, bname, vr))
        else:
            results.append(f"\u94f6\u884c\u5361\uff1a{bnum}\n\u67e5\u8be2\u5931\u8d25")
